...
{
	if (!g_user.GetSkeletonCap().IsTracking(player))
	{
		printf("not tracked!\n");
		return true;
	}

	if (!g_user.GetSkeletonCap().IsJointActive(eJoint1) ||
		!g_user.GetSkeletonCap().IsJointActive(eJoint2))
	{
		return false;
	}

	XnSkeletonJointPosition joint1, joint2;
	g_user.GetSkeletonCap().GetSkeletonJointPosition(player, eJoint1, joint1);
	g_user.GetSkeletonCap().GetSkeletonJointPosition(player, eJoint2, joint2);
	...
	g_depth.ConvertRealWorldToProjective(2, pt, pt);
#ifndef USE_GLES
...
void DrawJoint(XnUserID player, XnSkeletonJoint eJoint)
{
	...
	if (!g_user.GetSkeletonCap().IsJointActive(eJoint))
	{
		return;
	}

	XnSkeletonJointPosition joint;
	g_user.GetSkeletonCap().GetSkeletonJointPosition(player, eJoint, joint);
	...
	g_depth.ConvertRealWorldToProjective(1, &pt, &pt);
...
void DrawResult(){
	...
	if(rawDatas.size()>1){
		...
		g_depth.ConvertRealWorldToProjective(1, &leftHandPosition, &pt);
		xnOSStrFormat(angles, sizeof(angles), &nDummy, "%3.3f", rawDatas.back()->left_limb_angle);
		glRasterPos2i(pt.X, pt.Y);
		glPrintString(GLUT_BITMAP_HELVETICA_18, angles);

		g_depth.ConvertRealWorldToProjective(1, rawDatas.back()->joints+2, &pt);
		xnOSStrFormat(angles, sizeof(angles), &nDummy, "%3.3f", rawDatas.back()->left_arm_angle);
		glRasterPos2i(pt.X, pt.Y);
		glPrintString(GLUT_BITMAP_HELVETICA_18, angles);

		g_depth.ConvertRealWorldToProjective(1, &rightHandPosition, &pt);
		xnOSStrFormat(angles, sizeof(angles), &nDummy, "%3.3f", rawDatas.back()->right_limb_angle);
		glRasterPos2i(pt.X, pt.Y);
		glPrintString(GLUT_BITMAP_HELVETICA_18, angles);

		g_depth.ConvertRealWorldToProjective(1, rawDatas.back()->joints+5, &pt);
		xnOSStrFormat(angles, sizeof(angles), &nDummy, "%3.3f", rawDatas.back()->right_arm_angle);
		glRasterPos2i(pt.X, pt.Y);
		glPrintString(GLUT_BITMAP_HELVETICA_18, angles);
	}	

	glRasterPos2i(10, 50);
	glPrintString(GLUT_BITMAP_HELVETICA_18, lastSignName);
}
*/
...
void DrawDepthMap(const xn::DepthMetaData& dmd, const xn::SceneMetaData& smd)
{
	...
	{
		XnUInt32 nIndex = 0;
		XnSkeletonJointPosition torsoPosition;
		g_user.GetSkeletonCap().GetSkeletonJointPosition(g_userID, XN_SKEL_TORSO, torsoPosition);
		...
		for (nY=0; nY<g_nYRes; nY++)
		{
			for (nX=0; nX < g_nXRes; nX++, nIndex++)
			{
				...
				if (g_bDrawBackground || *pLabels != 0)
				{
					...
					if(label!=0 && g_user.GetSkeletonCap().IsTracking(g_userID) && nValue<torsoPosition.position.Z-150)
					{
						pDestImage[0] = 255;
						pDestImage[1] = 255;
						pDestImage[2] = 0;
	...
	for (int i = 0; i < nUsers; ++i)
	{
		...
		{
			XnPoint3D com;
			g_user.GetCoM(aUsers[i], com);
			g_depth.ConvertRealWorldToProjective(1, &com, &com);
			...
			else if (g_user.GetSkeletonCap().IsTracking(aUsers[i]))
			{
				// Tracking
				xnOSStrFormat(strLabel, sizeof(strLabel), &nDummy, "%d - Tracking", aUsers[i]);
			}
			else if (g_user.GetSkeletonCap().IsCalibrating(aUsers[i]))
			{
				// Calibrating
		...
		if (g_bDrawSkeleton && g_user.GetSkeletonCap().IsTracking(aUsers[i]))
		{
